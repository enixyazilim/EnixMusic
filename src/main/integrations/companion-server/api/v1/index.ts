...
const CompanionServerAPIv1: FastifyPluginCallback<CompanionServerAPIv1Options> = async (fastify, options) => {
  const sendCommand = (commandRequest: APIV1CommandRequestBodyType) => {
    const enixmView = options.getYtmView();
    if (enixmView) {
      switch (commandRequest.command) {
        ...
        case "play": {
          enixmView.webContents.send("remoteControl:execute", "play");
          break;
        }

        case "pause": {
          enixmView.webContents.send("remoteControl:execute", "pause");
          break;
        }

        case "volumeUp": {
          enixmView.webContents.send("remoteControl:execute", "volumeUp");
          break;
        }

        case "volumeDown": {
          enixmView.webContents.send("remoteControl:execute", "volumeDown");
          break;
        }
        ...
        case "setVolume": {
          ...
          enixmView.webContents.send("remoteControl:execute", "setVolume", volume);
          break;
        }

        case "mute": {
          enixmView.webContents.send("remoteControl:execute", "mute");
          break;
        }

        case "unmute": {
          enixmView.webContents.send("remoteControl:execute", "unmute");
          break;
        }
        ...
        case "seekTo": {
          ...
          enixmView.webContents.send("remoteControl:execute", "seekTo", position);
          break;
        }
        ...
        case "repeatMode": {
          ...
          switch (repeatMode) {
            case RepeatMode.None: {
              enixmView.webContents.send("remoteControl:execute", "repeatMode", "NONE");
              break;
            }
            case RepeatMode.All: {
              enixmView.webContents.send("remoteControl:execute", "repeatMode", "ALL");
              break;
            }
            case RepeatMode.One: {
              enixmView.webContents.send("remoteControl:execute", "repeatMode", "ONE");
              break;
            }
            default: {
        ...
        case "shuffle": {
          enixmView.webContents.send("remoteControl:execute", "shuffle");
          break;
        }
        ...
        case "playQueueIndex": {
          ...
          enixmView.webContents.send("remoteControl:execute", "playQueueIndex", index);
          break;
        }

        case "toggleLike": {
          enixmView.webContents.send("remoteControl:execute", "toggleLike");
          break;
        }

        case "toggleDislike": {
          enixmView.webContents.send("remoteControl:execute", "toggleDislike");
          break;
        }
      }
    }
    ...
    async (request, response) => {
      ...
      const authorizationWindow = new BrowserWindow({
        ...
        webPreferences: {
          ...
          preload: path.join(__dirname, '../preload/authorize_companion_window.js'),
          additionalArguments: [requestId, authData.appName, request.body.code, options.getStore().get("general.language") || "tr"]
        }
      });
  ...
  fastify.get(
    ...
    async (request, response) => {
      ...
      if (enixmView) {
        ...
        ipcMain.once(`enixmView:getPlaylists:response:${requestId}`, playlistsResponseListener);

        enixmView.webContents.send(`enixmView:getPlaylists`, requestId);

        await new Promise((_resolve, reject) =>
          setTimeout(() => {
            ipcMain.removeListener(`enixmView:getPlaylists:response:${requestId}`, playlistsResponseListener);
  ...
  fastify.ready().then(() => {
    ...
    ipcMain.on("enixmView:createPlaylistObserved", createPlaylistObservedListener);
    ...
    ipcMain.on("enixmView:deletePlaylistObserved", deletePlaylistObservedListener);
    ...
    fastify.addHook("onClose", () => {
      ...
      ipcMain.off("enixmView:createPlaylistObserved", createPlaylistObservedListener);
      ipcMain.off("enixmView:deletePlaylistObserved", deletePlaylistObservedListener);
    });
  });
};

...
// In development, electron-vite provides the dev server URL via ELECTRON_RENDERER_URL
const ALL_WINDOWS_VITE_DEV_SERVER_URL = process.env.ELECTRON_RENDERER_URL;

declare const ENIXM_DISABLE_UPDATES: boolean;
declare const ENIXM_UPDATE_FEED_OWNER: string;
declare const ENIXM_UPDATE_FEED_REPOSITORY: string;
...
let activeAdblocker: ElectronBlocker | null = null;
...
log.errorHandler.startCatching({
  ...
  onError({ error, processType, versions }) {
    ...
    if (!app.isReady()) {
      dialog.showErrorBox(`Enix Music Crashed`, `Application crashed before ready\n\n${dialogMessage}`);
    } else {
      ...
      if (result === 0 || result === 2) {
        clipboard.writeText(`Enix Music Crashed\n\n${dialogMessage}`);
      }
    }
...
// Disable Chromium touch UI mode to prevent oversized touch padding/spacing in menus
app.commandLine.appendSwitch("touch-events", "disabled");
app.commandLine.appendSwitch("disable-touch-adjustment", "1");
...
const template: MenuItemConstructorOptions[] = [{ role: "appMenu", label: "Enix Music" }, { role: "editMenu" }];
...
const enixmViewIntegrationScripts: { [name: string]: { [name: string]: string } } = {};

let mainWindow: BrowserWindow = null;
let settingsWindow: BrowserWindow = null;
let enixmView: BrowserView = null;
let tray: Tray = null;
...
let enixmViewLoadTimeout: NodeJS.Timeout | null = null;
      ...
      switch (paths[0]) {
        case "play": {
          if (paths.length >= 2) {
            ...
            if (enixmView) {
              enixmView.webContents.send("remoteControl:execute", "navigate", {
                watchEndpoint: {
                  videoId: videoId,
...
if (!app.isDefaultProtocolClient("enixmusic")) {
  if (process.defaultApp) {
    if (process.argv.length >= 2) {
      log.info("Application set as default protcol client for 'enixmusic'");
      app.setAsDefaultProtocolClient("enixmusic", process.execPath, [path.resolve(process.argv[1])]);
    }
  } else {
    log.info("Application set as default protcol client for 'enixmusic'");
    app.setAsDefaultProtocolClient("enixmusic", process.execPath);
  }
}
...
memoryStore.onStateChanged((newState, oldState) => {
  ...
  if (enixmView !== null) {
    enixmView.webContents.send("memoryStore:stateChanged", newState, oldState);
  }
});
log.info("Created memory store");

function shouldDisableUpdates(): boolean {
  // macOS can't have auto updates without a code signature; linux uses package managers
...
if (app.isPackaged && !shouldDisableUpdates() && !ENIXM_DISABLE_UPDATES) {
  ...
  autoUpdater.on("update-available", () => {
    ...
    if (appLaunchUpdateCheck) memoryStore.set("enixmViewLoadingStatus", getLoadingLocale().downloadingUpdate);
...
function getControlsIconPath(icon: string) {
  return getIconPath(`controls/${icon}`);
}

function anyShortcutChanged(newState: Readonly<StoreSchema>, oldState: Readonly<StoreSchema>): boolean {
  return (Object.keys(newState.shortcuts) as (keyof StoreSchema["shortcuts"])[]).some(
    key => newState.shortcuts[key] !== oldState.shortcuts[key]
  );
}
...
function getLoadingLocale() {
  const currentLang = (store.get("general.language") as Language) || "en";
  return getLocale(currentLang).loading;
}

function updateTrayMenu() {
  if (!tray) return;
  const currentLang = (store.get("general.language") as Language) || "en";
  const l = getLocale(currentLang).tray;
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (enixmView !== null) {
    enixmView.webContents.send("settings:stateChanged", newState, oldState);
  }

  if (newState.general.language !== oldState?.general?.language) {
    updateTrayMenu();
  }
  ...
  if (newState.general.adblockerEnabled && !oldState.general.adblockerEnabled) {
    ...
    if (activeAdblocker) {
      activeAdblocker.disableBlockingInSession(session.fromPartition(app.isPackaged ? "persist:enixmview" : "persist:enixmview-dev"));
  ...
  if (newState.appearance.zoom !== oldState.appearance.zoom) {
    if (enixmView) {
      enixmView.webContents.setZoomFactor(newState.appearance.zoom / 100);
  ...
  if (newState.appearance.customCSSEnabled) {
    customCss.provide(store, enixmView);
  }
  ...
  if (newState.integrations.companionServerEnabled) {
    companionServer.provide(store, memoryStore, enixmView);
  }
...
function updateThumbarButtons(state: PlayerState | null, force = false) {
  ...
  if (!force && lastThumbarState && lastThumbarState.isPlaying === isPlaying) {
    return;
  }
  lastThumbarState = { hasVideo: !!state?.videoDetails, isPlaying };
...
function setupTaskbarFeatures() {
  ...
  if (mainWindow && process.platform === "win32") {
    ...
    mainWindow.on("minimize", refreshThumbar);
  }
...
function registerShortcuts() {
  ...
  const shortcutDefs: {
    key: string;
    action: string;
    memoryKey: keyof MemoryStoreSchema;
    name: string;
  }[] = [
    { key: shortcuts.playPause, action: "playPause", memoryKey: "shortcutsPlayPauseRegisterFailed", name: "playPause" },
    { key: shortcuts.next, action: "next", memoryKey: "shortcutsNextRegisterFailed", name: "next" },
    { key: shortcuts.previous, action: "previous", memoryKey: "shortcutsPreviousRegisterFailed", name: "previous" },
    { key: shortcuts.thumbsUp, action: "toggleLike", memoryKey: "shortcutsThumbsUpRegisterFailed", name: "thumbsUp" },
    { key: shortcuts.thumbsDown, action: "toggleDislike", memoryKey: "shortcutsThumbsDownRegisterFailed", name: "thumbsDown" },
    { key: shortcuts.volumeUp, action: "volumeUp", memoryKey: "shortcutsVolumeUpRegisterFailed", name: "volumeUp" },
    { key: shortcuts.volumeDown, action: "volumeDown", memoryKey: "shortcutsVolumeDownRegisterFailed", name: "volumeDown" }
  ];

  for (const def of shortcutDefs) {
    if (!def.key) {
      memoryStore.set(def.memoryKey as string, false);
      continue;
    }

    let registered = false;
    try {
      registered = globalShortcut.register(def.key, () => {
        if (enixmView) {
          enixmView.webContents.send("remoteControl:execute", def.action);
        }
      });
    } catch {
      /* ignored */
    ...
    if (!registered) {
      log.info(`Failed to register shortcut: ${def.name}`);
      memoryStore.set(def.memoryKey as string, true);
    } else {
      log.info(`Registered shortcut: ${def.name}`);
      memoryStore.set(def.memoryKey as string, false);
    }
  }
...
// Functions with call to enixmView renderer
function enixmViewNavigated() {
  if (enixmView !== null) {
    const url = enixmView.webContents.getURL();
    if (url.startsWith("https://music.youtube.com/")) {
      lastUrl = url;
      enixmView.webContents.send("enixmView:navigationStateChanged", {
        canGoBack: enixmView.webContents.navigationHistory.canGoBack(),
        canGoForward: enixmView.webContents.navigationHistory.canGoForward()
      });
    }
  }
}
...
// Handles any navigation or window opening from enixmView
function openExternalFromEnixMView(urlString: string) {
...
const createOrShowSettingsWindow = (): void => {
  ...
  settingsWindow = new BrowserWindow({
    ...
    frame: false,
    show: false,
    icon: getIconPath(process.platform === "win32" ? "enixmusic.ico" : "enixmusic.png"),
    parent: mainWindow,
    modal: !isDarwin,
    ...
    webPreferences: {
      ...
      preload: path.join(__dirname, '../preload/settings_window.js'),
...
const createEnixMView = (): void => {
  ...
  enixmView = new BrowserView({
    webPreferences: {
      sandbox: true,
      contextIsolation: true,
      backgroundThrottling: false,
      partition: app.isPackaged ? "persist:enixmview" : "persist:enixmview-dev",
      preload: path.join(__dirname, '../preload/enixmview.js'),
  ...
  enixmView.webContents.setMaxListeners(30);

  companionServer.provide(store, memoryStore, enixmView);
  customCss.provide(store, enixmView);
  ratioVolume.provide(enixmView);

  // Attach events to enixm view
  enixmView.webContents.on("will-navigate", event => {
    const url = new URL(event.url);
    if (isPreventedNavOrRedirect(url)) {
      event.preventDefault();
      log.info(`Blocking EnixM View navigation to ${event.url}`);

      openExternalFromEnixMView(event.url);
    }
  });
  enixmView.webContents.on("will-redirect", event => {
    const url = new URL(event.url);
    if (isPreventedNavOrRedirect(url)) {
      event.preventDefault();
      log.info(`Blocking EnixM View redirect to ${event.url}`);
    }
  ...
  enixmView.webContents.on("did-navigate", enixmViewNavigated);
  enixmView.webContents.on("did-navigate-in-page", enixmViewNavigated);
  enixmView.webContents.on("enter-html-full-screen", () => {
    if (mainWindow) {
      mainWindow.setFullScreen(true);
    }
  });
  enixmView.webContents.on("leave-html-full-screen", () => {
    if (mainWindow) {
      mainWindow.setFullScreen(false);
    }
  });
  enixmView.webContents.on("render-process-gone", () => {
    store.set("state.lastUrl", lastUrl);
    store.set("state.lastVideoId", lastVideoId);
    store.set("state.lastPlaylistId", lastPlaylistId);
    createEnixMView();
  });
  enixmView.webContents.on("page-title-updated", (_event, title) => {
    if (mainWindow) {
      mainWindow.setTitle(title === "YouTube Music" ? "Enix Music" : `${title} | Enix Music`);
    }
  });
  enixmView.webContents.on("context-menu", (_event, params) => {
  ...
  enixmView.webContents.on("will-prevent-unload", event => {
    if (mainWindow) {
      if (!applicationQuitting) {
        if (enixmView.webContents.getURL().startsWith("https://music.youtube.com/")) {
  ...
  enixmView.webContents.on("unresponsive", () => {
    memoryStore.set("enixmViewUnresponsive", true);
  });
  enixmView.webContents.on("responsive", () => {
    memoryStore.set("enixmViewUnresponsive", false);
  });

  enixmView.webContents.setWindowOpenHandler(details => {
    openExternalFromEnixMView(details.url);
  ...
  enixmView.webContents.on("did-start-loading", () => {
    memoryStore.set("enixmViewLoadingStatus", getLoadingLocale().loadingApp);
  });

  enixmView.webContents.on("did-stop-loading", () => {
    if (!memoryStore.get("enixmViewLoadingError")) {
      memoryStore.set("enixmViewLoadingStatus", getLoadingLocale().loaded);
    }
  });
  ...
  enixmView.webContents.on("did-fail-load", (_event, errorCode, errorDescription, _validatedURL, isMainFrame) => {
    if (isMainFrame) {
      ...
      memoryStore.set("enixmViewLoadingError", true);
      memoryStore.set("enixmViewLoadingStatus", `${getLoadingLocale().loadFailed}: ${errorDescription} (${errorCode})`);
    }
  });

  memoryStore.set("enixmViewLoadingStatus", getLoadingLocale().started);
  ...
  if (continueWhereYouLeftOff) {
    ...
    if (lastUrl) {
      if (lastUrl.startsWith("https://music.youtube.com/")) {
        enixmView.webContents.loadURL(lastUrl);
  ...
  if (navigateDefault) {
    enixmView.webContents.loadURL("https://music.youtube.com/");
...
app.on("ready", async () => {
  ...
  try {
    ...
    if (activeAdblocker && store.get("general").adblockerEnabled) {
      ...
      log.info("Adblocker initialized but is disabled in settings.");
    }
  } catch (error) {
  ...
  ipcMain.on("enixmView:videoProgressChanged", (event, progress) => {
    if (event.sender !== enixmView.webContents) return;

    playerStateStore.updateVideoProgress(progress);
  });

  ipcMain.on("enixmView:videoStateChanged", (event, state) => {
    if (event.sender !== enixmView.webContents) return;

    // enixm state mapping definitions
    // -1 -> Unstarted
    // 1 -> Playing
    // 2 -> Paused
    // 3 -> Buffering
  ...
  ipcMain.on("enixmView:videoDataChanged", (event, videoDetails, playlistId, album, likeStatus, hasFullMetadata) => {
    if (event.sender !== enixmView.webContents) return;
  ...
  ipcMain.on("enixmView:storeStateChanged", (event, queue, likeStatus, volume, muted, adPlaying) => {
    if (event.sender !== enixmView.webContents) return;

    playerStateStore.updateFromStore(queue, likeStatus, volume, muted, adPlaying);
  });

  ipcMain.on("enixmView:switchFocus", (event, context) => {
    if (event.sender !== enixmView.webContents && event.sender !== mainWindow.webContents) return;

    if (context === "main") {
      if (mainWindow && enixmView.webContents.isFocused()) {
        mainWindow.webContents.focus();
      }
    } else if (context === "enixm") {
      if (enixmView && mainWindow.webContents.isFocused()) {
        enixmView.webContents.focus();
      }
    }
  });

  ipcMain.on("enixmView:navigateDefault", event => {
    if (enixmView) {
      if (event.sender !== mainWindow.webContents) return;
  ...
  ipcMain.on("enixmView:recreate", event => {
    if (event.sender !== mainWindow.webContents) return;

    if (enixmView) {
      if (mainWindow) {
        mainWindow.removeBrowserView(enixmView);
      }

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (enixmView.webContents as any).destroy();
      enixmView = null;
      createEnixMView();
    }
  });

  ipcMain.handle("enixmView:getIntegrationScripts", event => {
    if (event.sender !== enixmView.webContents) return;

    return enixmViewIntegrationScripts;
  });
  ...
  ipcMain.on("settings:set", (event, key: string, value?: unknown) => {
    ...
    if (value === undefined) {
      (store as Conf<Record<string, unknown>>).delete(key);
    } else {
      (store as Conf<Record<string, unknown>>).set(key, value);
    }
  });
  ...
  ipcMain.handle("settings:get", (event, key: string) => {
    if (
      ...
      enixmView &&
      event.sender !== enixmView.webContents
    )
      return;
  ...
  const enixmSession = session.fromPartition(app.isPackaged ? "persist:enixmview" : "persist:enixmview-dev");

  enixmSession.setPermissionCheckHandler((webContents, permission) => {
    if (webContents == enixmView.webContents) {
  ...
  session.fromPartition(app.isPackaged ? "persist:enixmview" : "persist:enixmview-dev").setPermissionRequestHandler((webContents, permission, callback) => {
    if (webContents == enixmView.webContents) {
      if (permission === "fullscreen") {
  ...
  memoryStore.set("enixmViewLoading", true);
  memoryStore.set("enixmViewLoadingStatus", getLoadingLocale().checkingForUpdates);
  ...
  enixmViewIntegrationScripts["ratioVolume"] = ratioVolume.getYTMScripts().reduce<{ [name: string]: string }>((map, obj) => {
  ...
  if (store.get("appearance").zoom) {
    log.info("Integration update: Zoom Factor");
    enixmView.webContents.setZoomFactor(store.get("appearance").zoom / 100);
  }

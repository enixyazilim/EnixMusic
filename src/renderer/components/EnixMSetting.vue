...
const props = defineProps<{
  ...
  flexColumn?: boolean;
  beta?: boolean;
  optionsMap?: Record<string | number, string>; // This is for the select menu
}>();
...
const selectedOption = computed(() => {
  if (!props.optionsMap || props.modelValue === undefined || props.modelValue === null) {
    return "";
  }
  return props.optionsMap[props.modelValue as string | number] ?? String(props.modelValue);
});

// Handles both string options (e.g. "tr", "en") and number options (e.g. TrayIconStyle)
function select(optionKey: string | number) {
  const isNumeric = typeof props.modelValue === "number" || (!isNaN(Number(optionKey)) && typeof props.optionsMap?.[Number(optionKey)] !== "undefined");
  value.value = (isNumeric ? Number.parseInt(String(optionKey)) : String(optionKey)) as ModelValue[T];
...
<template>
  <div :class="{ 'enixm-setting': true, 'indented': props.indented, 'flex-column': props.flexColumn }">
...
.enixm-setting.indented {
  margin-left: 12px;
  padding-left: 12px;
  border-left: 1px solid #212121;
}

.enixm-setting.flex-column {
  flex-direction: column;
  align-items: initial;
  justify-content: initial;
}

.enixm-setting .beta-tag {
  background-color: #f44336;
  border-radius: 4px;
  padding: 2px 4px;
}

.enixm-setting .disabled-tag {
